...
sub execute_command() {
    ...
    if ( length($FILENAME) > 0 ) {
        $COMMAND="wget -q --show-progress -P "/content/drive/$var_location" -S --no-check-certificate --spider --load-cookie $TEMP --save-cookie $TEMP \"$URL\" 2>&1";
        my @HEADERS=`$COMMAND`;
    ...
    $COMMAND="wget $CONTINUE -q --show-progress -P "/content/drive/$var_location" --progress=dot:giga --no-check-certificate --load-cookie $TEMP --save-cookie $TEMP \"$URL\"";
